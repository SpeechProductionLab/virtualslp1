...
function instruction1Play(){
	if (task1.paused){
		task1.play();
		button1.removeAttr('id');
		button1.attr('id', 'pauseBtn1');
	}else{
		task1.pause();
		button1.removeAttr('id');
		button1.attr('id', 'readBtn1');
	}
}
task1.addEventListener('ended', function(){
	button1.removeAttr('id');
...
$('#recButton').click(function () {
	if ($('#recButton').hasClass('btn-video-rec')) {
		...
		timeouts.push( setTimeout(function () {
			...
			document.getElementById("recButton").disabled = true;
			document.getElementById("recButton").style.opacity = 0.3;
		}, 125000) ); // set max video recording to 1:35 minutes
	}
	else {
		$('#recButton').removeClass("btn-video-stop");
		$('#recButton').addClass("btn-video-rec");
...
var lst_video = [5, 9];
var lst_task = ['-1', '0', '1', '2', '3', 'DDK_PA', '5', 'VWL_SUSTAIN', '7', 'PSG_BAMBOO'];
...
if (obj.isMobile) {
	...
	window.location.href = "https://www.cs.toronto.edu/~taati/virtualSLP/index_mobile.html";
}
	...
	xhr.upload.addEventListener("progress", function (event) {
		if (event.lengthComputable) {
			...
			if (complete == 100) {
				...
				detectMediaMain();
				// yourVideo.volume = 0;
				// yourVideo.srcObject = tempCamera;
				setTimeout(function () {
					document.getElementById("recButton").disabled = false;
					document.getElementById("recButton").style.opacity = 1;
...
// code related to face detection
/*
canvas_api = document.getElementById('c-sample-api');
c_api = document.getElementById('c-api');
yourVideoSample.addEventListener('play', () => {
	//const canvas_api = faceapi.createCanvasFromMedia(yourVideoSample)
	// canvas_api.style.zIndex = "11";
  	//document.getElementsByClassName('video-sample-container')[0].append(canvas_api)
	const displaySize = { width: yourVideoSample.width, height: yourVideoSample.height }
	faceapi.matchDimensions(canvas_api, displaySize)
	setInterval(async () => {
    const detections = await faceapi.detectAllFaces(yourVideoSample, new faceapi.TinyFaceDetectorOptions()).withFaceLandmarks()
    const resizedDetections = faceapi.resizeResults(detections, displaySize)
    canvas_api.getContext('2d').clearRect(0, 0, canvas_api.width, canvas_api.height)
    faceapi.draw.drawDetections(canvas_api, resizedDetections)
	faceapi.draw.drawFaceLandmarks(canvas_api, resizedDetections)
	//console.log(detections);
	//console.log(detections[0].box.x)
	//console.log(detections[0].box.width)
	if (typeof detections[0] !== "undefined") {
		if (detections[0].box.y < 100 | detections[0].box.x < 200 | (detections[0].box.x + detections[0].box.width) > 750 | (detections[0].box.height + detections[0].box.y) > 650) {
			context_sample.strokeStyle = "#DC143C";
			// context_sample.lineWidth = 4;
			context_sample.stroke();
		} else {
			context_sample.strokeStyle = "#7FFF00";
			// context_sample.lineWidth = 4;
			context_sample.stroke();
		}
	}
    // faceapi.draw.drawFaceLandmarks(canvas_api, resizedDetections)
    //faceapi.draw.drawFaceExpressions(canvas_api, resizedDetections)
	// const landmarks = await faceapi.detectFaceLandmarks(yourVideoSample)
	...
	// console.log("Left eye brow position ===>" + JSON.stringify(leftEyeBrow));
	// console.log("right eye brow position ===>" + JSON.stringify(rightEyeBrow));
	// console.log("Jaw outline position ===>" + JSON.stringify(jawOutline));
	//console.log(jawOutline);
	//console.log(leftEyeBrow);
	//console.log(rightEyeBrow);
	}, 200)
});
*/
/*
yourVideo.addEventListener('play', () => {
	if (yourVideo.srcObject != null) {
		const displaySize_new = { width: yourVideo.width, height: yourVideo.height}
		faceapi.matchDimensions(c_api, displaySize_new)
		setInterval(async () => {
		const detections_new = await faceapi.detectAllFaces(yourVideo, new faceapi.TinyFaceDetectorOptions()).withFaceLandmarks()
		const resizedDetections_new = faceapi.resizeResults(detections_new, displaySize_new)
		c_api.getContext('2d').clearRect(0, 0, c_api.width, c_api.height)
		faceapi.draw.drawDetections(c_api, resizedDetections_new)
		faceapi.draw.drawFaceLandmarks(c_api, resizedDetections_new)
		if (typeof detections_new[0] !== "undefined") {
			if (detections_new[0].box.y < 100 | detections_new[0].box.x < 200 | (detections_new[0].box.x + detections_new[0].box.width) > 750 | (detections_new[0].box.height + detections_new[0].box.y) > 650) {
...
//var lookupTable = $.getJSON("lookup.json");
...
var lookupTable;
$.getJSON("./js/lookup.json", function (lookupTableFinal) {
